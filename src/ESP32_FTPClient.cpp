...
ESP32_FTPClient::ESP32_FTPClient(String _serverAdress, uint16_t _port, String _userName, String _passWord, uint16_t _timeout, uint8_t _verbose){
  userName = _userName;
  passWord = _passWord;
...
ESP32_FTPClient::ESP32_FTPClient(String _serverAdress, String _userName, String _passWord, uint16_t _timeout, uint8_t _verbose){
  userName = _userName;
  passWord = _passWord;
  serverAdress = _serverAdress;
  port = 21;
...
WiFiClientSecure* ESP32_FTPClient::GetDataClient() {
  return &dclient;
}
...
void ESP32_FTPClient::GetLastModifiedTime(const char  * fileName, char* result) {
  ...
  SendCommand(String(F("MDTM ")) + F(fileName));
  GetFTPAnswer (result, 4);
}

void ESP32_FTPClient::WriteClientBuffered(WiFiClientSecure* cli, unsigned char * data, int dataLength) {
  if(!isConnected()) return;
...
void ESP32_FTPClient::SendCommand(String command) {
  FTPdbg(F("Sending: "));
  FTPdbgn(command);
  String lf("\r\n");
  client.print(command + lf);
}
...
void ESP32_FTPClient::OpenConnection() {
  ...
  client.setInsecure();
  if( client.connect(serverAdress.c_str(), port, timeout) )
  {
  ...
  FTPdbg("Send: ");
  FTPdbgn(String(F("USER ")) + userName);
  SendCommand(String(F("USER ")) + userName);
  GetFTPAnswer();

  FTPdbg("Send: ");
  FTPdbgn(String(F("PASS ")) + passWord);
  SendCommand(String(F("PASS ")) + passWord);
  GetFTPAnswer();
  
  FTPdbg("Send: ");
  FTPdbgn(String(F("PBSZ 0")));
  SendCommand(String(F("PBSZ 0")));
  GetFTPAnswer();
  FTPdbg("Send: ");
  FTPdbgn(String(F("PROT P")));
  SendCommand(String(F("PROTO P")));
  GetFTPAnswer();
  FTPdbg("Send: ");
  FTPdbgn(String(F("PWD")));
  SendCommand(String(F("PWD")));
  GetFTPAnswer();
...
void ESP32_FTPClient::RenameFile(char* from, char* to) {
  FTPdbgn("Send RNFR");
  if(!isConnected()) return;
  SendCommand(String(F("RNFR ")) + F(from));
  GetFTPAnswer();

  FTPdbgn("Send RNTO");
  SendCommand(String(F("RNTO ")) + F(to));
  GetFTPAnswer();
}
...
void ESP32_FTPClient::NewFile (const char* fileName) {
  FTPdbgn("Send STOR");
  if(!isConnected()) return;
  SendCommand(String(F("STOR ")) + F(fileName));
  GetFTPAnswer();
}
...
void ESP32_FTPClient::AppendFile (char* fileName) {
  FTPdbgn("Send APPE");
  if(!isConnected()) return;
  SendCommand(String(F("APPE ")) + F(fileName));
  GetFTPAnswer();
}
...
void ESP32_FTPClient::ChangeWorkDir(const char * dir) {
  FTPdbgn("Send CWD");
  if(!isConnected()) return;
  SendCommand(String(F("CWD ")) + F(dir));
  GetFTPAnswer();
}
...
void ESP32_FTPClient::DeleteFile(const char * file) {
  FTPdbgn("Send DELE");
  if(!isConnected()) return;
  SendCommand(String(F("DELE ")) + F(file));
  GetFTPAnswer();
}
...
void ESP32_FTPClient::MakeDir(const char * dir) {
  FTPdbgn("Send MKD");
  if(!isConnected()) return;
  SendCommand(String(F("MKD ")) + F(dir));
  GetFTPAnswer();
}
...
void ESP32_FTPClient::ContentList(const char * dir, String * list) {
  ...
  FTPdbgn("Send MLSD");
  if(!isConnected()) return;
  SendCommand(String(F("MLSD")) + F(dir));
  GetFTPAnswer(_resp);
...
void ESP32_FTPClient::ContentListWithListCommand(const char * dir, String * list) {
  ...
  FTPdbgn("Send LIST");
  if(!isConnected()) return;
  SendCommand(String(F("LIST ")) + F(dir));
  GetFTPAnswer(_resp);
  ...
  while(dclient.available()) 
  {
    if( _b < 128 )
    {
      ...
      list[_b] = tmp; // tmp.substring(tmp.lastIndexOf(" ") + 1, tmp.length());
...
void ESP32_FTPClient::DownloadString(const char * filename, String &str) {
  FTPdbgn("Send RETR");
  if(!isConnected()) return;
  SendCommand(String(F("RETR ")) + F(filename));
