...
void xtouch_loadTouchConfig(XTouchPanelConfig &config)
{
    ...
    if (error)
        ConsoleError("[XTouch][Touch]", "Failed to read touch config");
...
{
    ConsoleInfo("[XTouch][FS]", "Resetting touch config");
...
bool hasTouchConfig()
{
    ConsoleInfo("[XTouch][FS]", "Checking for touch config");
...
void xtouch_touch_setup()
{
    if (hasTouchConfig())
    {
        ConsoleInfo("[XTouch][Touch]", "Load from disk");
        xtouch_loadTouchConfig(x_touch_touchConfig);
    }
    else
    {
        ConsoleInfo("[XTouch][Touch]", "Touch Setup");
        TS_Point p;
        int16_t x1, y1, x2, y2;
